...
interface Playbook {
  /** Hostname suffixes this playbook applies to (matched case-insensitively). */
  match: string[];
  hints: string;
}
...
const PLAYBOOKS: Playbook[] = [
  {
    match: ["youtube.com"],
    hints:
      "Video titles/channels live deep in shadow DOM — call read_page (it pierces shadow roots) then `act` by the visible title text. The search box is named \"Search\"; submit with Enter. Results load lazily, so scroll then read_page again if a title isn't listed yet.",
  },
  {
  ...
  },
  {
    ...
    hints:
      "Infinite-scroll feed with content in shadow DOM — use read_page (viewport-first) and scroll to load more. The compose box is a contenteditable editor, so `act` with action:\"fill\" works (it routes through the editing pipeline). Buttons are aria-labelled (Like, Repost, Reply).",
  },
  {
    match: ["reddit.com"],
    hints:
      "New Reddit nests posts in shadow DOM and lazy-loads on scroll — read_page then scroll+read again for more. Comment/post boxes are contenteditable: use `act` fill. Prefer old.reddit.com if a flow keeps failing — its plain DOM is far easier.",
  },
  {
    match: ["github.com"],
    hints:
      "Many toggles (star, watch, merge) are React buttons that ignore synthetic clicks — `act`/click already falls back to a real DOM click, so trust it and re-read rather than re-clicking. Code search and the command palette (press \"/\") are faster than hunting the tree.",
  },
  {
    match: ["docs.google.com", "notion.so"],
    hints:
      "The editing surface is a rich editor (Google Docs paints to a canvas; Notion is contenteditable blocks). For Notion, `act` fill works on a focused block. For Google Docs the document body has no addressable DOM text — use the menus/toolbar by their labels, and type into the focused canvas via type_text.",
  },
  {
    match: ["linkedin.com", "facebook.com"],
    hints:
      "Heavy shadow DOM + lazy loading + frequent modal overlays. read_page (viewport-first) after each scroll; dismiss \"see more\"/cookie/login modals first (look for a close \"×\" or \"Not now\"). Post composers are contenteditable — `act` fill.",
  },
];
 ...
 */
export function getPlaybook(url: unknown): string {
