...
export type InflightMap = Map<string, number>; // requestId → start timestamp (ms)

/** Record a request entering flight. Idempotent for redirect re-fires. */
export function markRequestStart(map: InflightMap, requestId: string, now: number): void {
...
export function markRequestEnd(map: InflightMap, requestId: string): void {
  if (requestId == null) return;
 ...
 * terminal CDP event.
 */
export function sweepStale(map: InflightMap, now: number, maxAgeMs: number): number {
  let swept = 0;
...
export function isIdle(
  map: InflightMap,
  now: number,
  opts: { threshold?: number; maxAgeMs?: number; longLivedMs?: number } = {},
): boolean {
  ...
  for (const start of map.values()) {
    if (now - start < longLivedMs) young++;
  }
  return young <= threshold;
}

/** Current in-flight count after sweeping. Convenience for diagnostics. */
export function inflightCount(map: InflightMap, now: number, maxAgeMs = 15_000): number {
  sweepStale(map, now, maxAgeMs);
  return map.size;
}
