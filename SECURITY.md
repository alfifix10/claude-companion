...
- **Hard-coded tool allowlist.** `host/native-host.js` forces `--allowedTools mcp__claude-companion__* mcp__*__* Read Grep Glob WebFetch WebSearch` on every `claude` spawn and ignores any caller-supplied list. The two MCP patterns cover our own server explicitly plus every MCP server the user has registered via `claude mcp add` — the single wildcard `mcp__*` looks tempting but does NOT match tools from other servers because the CLI treats `__` as a segment separator. `Bash`, `Write`, `Edit`, `NotebookEdit` are on an explicit disallowlist. The `--dangerously-skip-permissions` flag is **not** used.
