...
- **Hard-coded tool disallowlist.** `host/native-host.js` forces `--disallowedTools Bash Write Edit NotebookEdit` on every `claude` spawn. This filter is respected even under `--dangerously-skip-permissions`, so a compromised prompt cannot get shell or filesystem access via the CLI. The extension itself never exposes those tools — the disallowlist is a belt-and-suspenders.
- **Non-interactive mode.** The CLI is always spawned with `-p` (headless) plus `--dangerously-skip-permissions` because there is no human at the CLI to answer permission prompts. Without skipping, Claude verbalises "I need approval" instead of running the tool and the user sees an infinite approval loop. The meaningful security gate is the disallowlist above, not the prompt.
