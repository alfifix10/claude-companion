...
- **Hard-coded tool allowlist.** `host/native-host.js` forces `--allowedTools mcp__* Read Grep Glob WebFetch WebSearch` on every `claude` spawn and ignores any caller-supplied list. The `mcp__*` wildcard covers every MCP server the user has explicitly registered via `claude mcp add` (their trusted computing base); narrowing it to our server alone broke legitimate workflows without a matching security win. `Bash`, `Write`, `Edit`, `NotebookEdit` are on an explicit disallowlist. The `--dangerously-skip-permissions` flag is **not** used.
